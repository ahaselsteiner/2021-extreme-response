...
%             3.2e+06  .* hs ./ (1 + 0.05 .* (tp - 3)))
        ...
        sigma = @(v1hr, hs, tp) 0 + (tp >= sqrt(2 * pi .* hs ./ (9.81 .* 1/14.99))) .* ...
            ((v1hr <= 25) .* (1.6064e+06 .* v1hr + -57816 .* v1hr.^2) + ...
            (v1hr > 25) .* 7.2898e+06 + ...
            1.7079e+05 .* hs.^2 ./ (1 + 0.3 ./ (1 + 5 .* (tp - 3).^2)));
        mu = @(v1hr, hs, tp) (tp >= sqrt(2 * pi .* hs ./ (9.81 .* 1/14.99))) .* ...
            ((((v1hr <= 25) .* (3.2968e+06 .* v1hr + 7.0863e+07 ./ (1 + 0.067621 * (v1hr - 11.4).^2)) + ...
            (v1hr > 25 ) .* (44021 .* v1hr.^2)).^2.0 + ...
            ((1 + (v1hr > 25) * 0.2) .* 9.7205e+06 .* hs.^1.0 .* (1 + 0.3 ./ (1 + 5 .* (tp - 3).^2))).^2.0).^(1/2.0));
